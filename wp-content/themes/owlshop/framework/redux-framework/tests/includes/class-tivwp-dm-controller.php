...
class TIVWP_DM_Controller {
    ...
    public static function construct() {
        ...
        if (!did_action('plugins_loaded')) {
            _doing_it_wrong(__METHOD__, __('Must call in of after the "plugins_loaded" action.','owlshop'), '14.03.19');
            return;
        }
    ...
    private static function _setup_automatic_switch() {
        ...
        if (!is_callable(array(
            ...
            TIVWP_DM_Notices::add(sprintf(__('Unknown action "%1$s" specified for %2$s', 'owlshop'), $action, 'TIVWP_DM_AUTO'), TIVWP_DM_Notices::WITH_TRIGGER_ERROR
            );

            $action = TIVWP_DM::ACTION_DEACTIVATE;

            TIVWP_DM_Notices::add(sprintf(__('The unknown action has been replaced with "%s"', 'owlshop'), $action), TIVWP_DM_Notices::WITH_TRIGGER_ERROR);
        }
         ...
         */
        TIVWP_DM_Notices::add(sprintf(__('Automatic action performed: "%s"', 'owlshop'), $action, TIVWP_DM_Notices::WITHOUT_TRIGGER_ERROR));
    }
