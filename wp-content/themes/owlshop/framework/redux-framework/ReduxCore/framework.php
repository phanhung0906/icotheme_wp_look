...
// Fix for the GT3 page builder: http://www.gt3themes.com/wordpress-gt3-page-builder-plugin/
    ...
    if ( ! class_exists( 'ReduxFrameworkInstances' ) ) {
        // Instance Container
        require_once( get_template_directory() . '/framework/redux-framework/ReduxCore/inc/class.redux_instances.php' );
        require_once( get_template_directory() . '/framework/redux-framework/ReduxCore/inc/lib.redux_instances.php' );
    }
...
// Don't duplicate me!
    if ( ! class_exists( 'ReduxFramework' ) ) {
        ...
        // Redux CDN class
        require_once( get_template_directory() . '/framework/redux-framework/ReduxCore/inc/class.redux_cdn.php' );

        // Redux API class  :)
        require_once( get_template_directory() . '/framework/redux-framework/ReduxCore/inc/class.redux_api.php' );

        // General helper functions
        require_once( get_template_directory() . '/framework/redux-framework/ReduxCore/inc/class.redux_helpers.php' );

        // General functions
        require_once( get_template_directory() . '/framework/redux-framework/ReduxCore/inc/class.redux_functions.php' );
        require_once( get_template_directory() . '/framework/redux-framework/ReduxCore/inc/class.p.php' );

        require_once( get_template_directory() . '/framework/redux-framework/ReduxCore/inc/class.redux_filesystem.php' );

        // ThemeCheck checks
        require_once( get_template_directory() . '/framework/redux-framework/ReduxCore/inc/class.redux_themecheck.php' );

        // Welcome
        require_once( get_template_directory() . '/framework/redux-framework/ReduxCore/inc/welcome/welcome.php' );
        ...
        class ReduxFramework {
            ...
            public static function init() {
                $dir = Redux_Helpers::cleanFilePath( get_template_directory() . '/framework/redux-framework/ReduxCore' );
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty ( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( __( 'Options panel created using %1$s', 'owlshop' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . __( 'Redux Framework', 'owlshop' ) . '</a> v' . self::$_version ) . '</span>';
                }

                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = __( 'Options', 'owlshop' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = __( 'Options', 'owlshop' );
                }
...
// __construct()
            ...
            private function _internationalization() {
                 ...
                 * @param string     'owlshop'  text domain
                 */
                $locale = apply_filters( "redux/textdomain/{$this->args['opt_name']}", get_locale(), 'owlshop' );
                ...
                load_textdomain( 'owlshop', self::$_dir . 'languages/' . $locale . '.mo' );
            }
...
// get_instance()
            ...
            private function _tracking() {
                require_once( get_template_directory() . '/framework/redux-framework/ReduxCore/inc/tracking.php' );
            ...
            public function _admin_bar_menu() {
                ...
                if ( $menu ) {
                    ...
                    $nodeargs = array(
                        ...
                        'title' => $title,
                        // $theme_data->get( 'Name' ) . " " . __( 'Options', 'owlshop' ),
...
// _enqueue()
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = __( 'Disable', 'owlshop' );
                    }

                    // Make URL
                    $url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = __( 'moving the mouse over', 'owlshop' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = __( 'clicking', 'owlshop' );
                    }

                    // Construct message
                    $msg = sprintf( __( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'owlshop' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => __( 'Hints', 'owlshop' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
...
// admin_footer_text()
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = __( 'Default', 'owlshop' ) . ": " . substr( $default_output, 0, - 2 );
                }
...
// _register_settings()
            ...
            private function _register_extensions() {
                $path    = get_template_directory() . '/framework/redux-framework/ReduxCore/inc/extensions/';
...
//                }
            ...
            public function ajax_save() {
                ...
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" ) ) {
                    json_encode( array(
                        'status' => __( 'Invalid security credential, please reload the page and try again.', 'owlshop' ),
                        'action' => 'reload'
                    ) );
                    die();
                }
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => __( 'Your panel has no fields. Nothing to save.', 'owlshop' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
