...
class cmb_Meta_Box {
	...
	public static function autoload_helpers( $class_name ) {
		...
		$dir = get_template_directory() . '/framework/metabox';
	...
	public function register_scripts() {
			...
			if ( ! is_admin() ) {
		   	...
		   	wp_register_script( 'wp-color-picker', admin_url( 'js/color-picker.min.js' ), array( 'iris' ), self::CMB_VERSION );
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => __( 'Clear' , 'owlshop' ),
					'defaultString' => __( 'Default' , 'owlshop'),
					'pick'          => __( 'Select Color' , 'owlshop'),
					'current'       => __( 'Current Color', 'owlshop' ),
				) );
			}
		} else {
		...
		wp_localize_script( 'cmb-scripts', 'cmb_l10', apply_filters( 'cmb_localized_data', array(
			...
			'up_arrow'        => '[ &uarr; ]&nbsp;',
			'down_arrow'      => '&nbsp;[ &darr; ]',
			'check_toggle'    => __( 'Select / Deselect All', 'owlshop' ),
		) ) );
	...
	public static function get_meta_box_url() {
		...
		if ( strtoupper( substr( PHP_OS, 0, 3 ) ) === 'WIN' ) {
			...
			$content_url = str_replace( $content_dir, WP_CONTENT_URL, get_template_directory() . '/framework/metabox' );
		  ...
		  $cmb_url = str_replace(
				...
				get_template_directory() . '/framework/metabox'
			);
		}
...
function cmb_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, __( 'Save' ,'owlshop' ) );
